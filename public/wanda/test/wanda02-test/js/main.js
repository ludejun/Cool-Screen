...
function drawLine2(ctx, arr, zoomx, zoomy, fn){
	...
	var a = Ani.add(function(t, arg){
		drawTrack2(ctx, arr, t / d, zoomx, zoomy, 'rgba(255,174,0,0.5)');
	}, d, function(){
		arr.done = 1;
		a.restart();
		// fn(a);
...
function drawTrack2(ctx, arr, per, zoomx, zoomy, color){
	...
	if(per > 0){
		...
		MT.EACH(arr.buffer, function(p, i, a){
			ctx.beginPath();
			ctx.arc(p.x * zoomx, p.y * zoomy, radio * Math.pow((a.length - i) / a.length, 1.5), 0, Math.PI * 2);
			ctx.fillStyle = color;
			ctx.fill();
		});
		...
		ctx.beginPath();
		ctx.moveTo(path[0].x * zoomx, path[0].y * zoomy);
		for(var c = 1; c < path.length; c++){
			ctx.lineTo(path[c].x * zoomx, path[c].y * zoomy);
		}
		ctx.strokeStyle = color;
...
function Draw(data){
	MT.EACH(data, function(v, i){
		// setTimeout(function(){
			drawLine2(ctx, v.coordinates, ZOOMX, ZOOMY, function onend(a){
				// a.restart();
				// pageIndex = (pageIndex + 1) % data.length;
				// var arr = data[pageIndex].coordinates;
				// drawLine2(ctx, arr, ZOOMX, ZOOMY, onend);
			});
		// }, delay += 60);
	});
	return;
	if(data.length <= pageSize){
		...
		MT.EACH(data, function(v, i){
			// setTimeout(function(){
				drawLine2(ctx, v.coordinates, ZOOMX, ZOOMY, function(){});
			// }, delay += 60);
		});
	} else {
		...
		MT.EACH(d, function(v, i){
			// setTimeout(function(){
				drawLine2(ctx, v.coordinates, ZOOMX, ZOOMY, function onend(){
					pageIndex = (pageIndex + 1) % data.length;
					var arr = data[pageIndex].coordinates;
					drawLine2(ctx, arr, ZOOMX, ZOOMY, onend);
				});
