                    ...
                    <g
                        ...
                        <polyline points="1730,890 210,890 210,260 " stroke="#00BFF2" strokeWidth="5"/> {yArray.map((it, index, array) => {
                            return <text
                                ...
                                key={index}
                                x={(minX - 30).toString()}
                                y={(maxY - ySpan * index).toString()}>{it}</text>
                        })
}
                        <text
                            className="left-degree"
                            x={(minX - 40).toString()}
                            y={(minY - 10).toString()}>次数</text>
                        {xArray.map((it, index, array) => {
                            return <text
                                className="left-degree"
                                key={index}
                                x={(minX + index * xSpan).toString()}
                                y={(maxY + 30).toString()}>{it}</text>
                        })
}
                        <text
                            className="left-degree"
                        ...
                        {ballArray.map((it, index) => {
                            return <g  key={index} className="circle-dump">
                                ...
                                stroke="none"
                                fill="url(#grad1)" />
                                <text x={(it.x * xPer + minX - 50).toString()}  y={(maxY - it.y * yPer).toString()} className="left-degree">{it.chinese}</text>
                                <text x={(it.x * xPer + minX - 50).toString()}  y={(maxY - it.y * yPer + 20).toString()} className="english-degree" textLength="47" lengthAdjust="spacingAndGlyphs">{it.english}</text>
                                </g>
                        })
}
                    </g>
