...
const minX = 210;
const maxX = 1730;
const ySpan = (maxY - minY) / 10; // yArray.length;
const xSpan = (maxX - minX) / 30; // xArray.length;
const xPer = xSpan / 2;
const yPer = ySpan / 1;
        ...
        >
          ...
          {circle.map((it, index) =>
            <circle
              ...
              fill={it.filter ? 'url(#grad2)' : it.linear ? 'url(#grad1)' : it.color.toString()}
            />
          )}
          <path
            opacity="0.301856884"
          ...
          <text className="left-degree" x={(maxX - 18 + 50).toString()} y={(maxY + 30).toString()}>
            时常（分钟）
          </text>
          {ballArray.map((it, index) =>
            ...
            >
              ...
              <circle
                ...
                r={it.width.toString()}
                stroke="none"
                filter={`${!it.linear ? 'url(#filter-10)' : 'url(#filter-4)'}`}
